...
BASE_DIR = "/workspace/data/data_push_11_14_relative"
...
def process_episode(episode_dir):
    ...
    # Second pass: output ABSOLUTE poses (corrected to camera frame)
    # The relative transformation will be computed dynamically during training
    # using relative_action_wrapper.py (UMI-style relative actions)
    out_rows = []
    for sec, nsec, T in reoriented_transforms:
        x, y, z, qx, qy, qz, qw = extract_pose_from_transformation(T)
        out_rows.append([sec, nsec, x, y, z, qx, qy, qz, qw])
    ...
    print(f"  ✓ {episode_dir.name}: Converted {len(out_rows)} rows (absolute poses, camera frame)")
    return True, False

def main():
    parser = argparse.ArgumentParser(description='Convert NDI poses to absolute poses in camera frame')
    parser.add_argument('--base-dir', type=str, default=BASE_DIR,
                        help=f'Base directory containing episode folders (default: {BASE_DIR})')
